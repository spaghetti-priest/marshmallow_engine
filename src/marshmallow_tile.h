...
typedef struct Tile {
   V2S min;       // Size of tile in screen or world space coordinates (depending on renderer)
   V2S max;
...
typedef struct Tilemap {
   int tile_width;
   int tile_height;
   // @Cleanup: If tilemap is square then tile dim is perfectly fesiable
   int tile_count_x;
   int tile_count_y;
...
function Tilemap *initialize_tilemap(Arena *arena, s32 tile_width, s32 tile_height, u32 tile_count_x, u32 tile_count_y, int start_x, int start_y, V4 color);
